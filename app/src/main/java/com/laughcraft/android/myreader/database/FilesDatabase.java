...
@Database(entities = {RecentFile.class,
   Bookmark.class}, exportSchema = false, version = 1)
public abstract class FilesDatabase extends RoomDatabase {
   ...
   public static synchronized FilesDatabase getInstance(Context context) {
      if (instance == null) {
         instance = Room.databaseBuilder(context.getApplicationContext(), FilesDatabase.class, DB_NAME).build();
      }
      return instance;
   }
   ...
   public abstract FavoriteFilesDao favoriteFilesDao();
}
